// ============================================================
// CONFIGURATION — Legal Metrology Compliance Checker
// Problem Statement: SIH26034 (DoCA / MoCA)
// ============================================================

// Read dynamic backend environment config if injected, else use sensible defaults
const env = (typeof window !== 'undefined' && window.ENV_CONFIG) ? window.ENV_CONFIG : {};

const DEFAULT_BACKEND_URL = (typeof window !== 'undefined' && window.location && window.location.origin && window.location.origin.startsWith('http')) 
    ? window.location.origin 
    : 'http://localhost:3000';

const CONFIG = {
    // Dynamic Backend URL (from .env via server injection or current host)
    BACKEND_URL: env.BACKEND_URL || DEFAULT_BACKEND_URL,

    // Backend API Endpoints (All route through BACKEND_URL)
    ...
    // Supabase Backend (Configured from .env or fallback)
    SUPABASE_URL: env.SUPABASE_URL || 'https://bsajwevjuuvgobaiouuc.supabase.co',
    SUPABASE_ANON_KEY: env.SUPABASE_ANON_KEY || 'sb_publishable_7QJ8KsPhW7Rw__emdD-axA_r4VfezAx',
...
// Auto fetch and sync backend config if not already injected
if (typeof window !== 'undefined') {
    fetch(CONFIG.CONFIG_API_URL)
        .then(res => res.ok ? res.json() : null)
        .then(data => {
            if (data) {
                if (data.backendUrl) CONFIG.BACKEND_URL = data.backendUrl;
                if (data.supabaseUrl) CONFIG.SUPABASE_URL = data.supabaseUrl;
                if (data.supabaseAnonKey) CONFIG.SUPABASE_ANON_KEY = data.supabaseAnonKey;
            }
        })
        .catch(err => {
            // Silently fallback to predefined values
        });
}
